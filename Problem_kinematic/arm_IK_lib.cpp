...
bool result = false;
  ...
  //  Serial.println(dxl_delta_move[0]);
  ...
  if ((abs(pos_x - next_pos_x) > 0) || ((abs(pos_z - next_pos_z) > 0)))
  {
    ...
    for (int i = 1; i < DXL_CNT - 2; i++)
    {
      ...
      Serial.println((int)((dxl_delta_move_med / dxl_delta_move[i] ) * goal_speed[sync_order(i)]));
    }
  }
  ...
  {
    Serial.println(log);
    Serial.println("Failed to sync adapting_speed");
  }
...
/*
Serial.println("Set all position in 0");
  ...
  delay(3000);
  Serial.println("End set all position in 0");
  */
}
